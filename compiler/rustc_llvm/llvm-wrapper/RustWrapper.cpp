...
LLVMRustGetInstrProfIncrementIntrinsic(LLVMModuleRef M) {
  return wrap(llvm::Intrinsic::getDeclaration(
      unwrap(M), (llvm::Intrinsic::ID)llvm::Intrinsic::instrprof_increment));
}
...
extern "C" LLVMValueRef LLVMRustBuildInvoke(
  ...
  return wrap(unwrap(B)->CreateInvoke(
      FTy, Callee, unwrap(Then), unwrap(Catch),
      ArrayRef<Value *>(unwrap(Args), NumArgs),
      ArrayRef<OperandBundleDef>(*OpBundles, NumOpBundles), Name));
}
...
extern "C" void LLVMAppendGlobalCtor(LLVMModuleRef M, LLVMValueRef Fn) {
  Function *F = unwrap<Function>(Fn);
